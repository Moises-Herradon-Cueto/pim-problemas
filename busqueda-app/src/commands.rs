...
pub async fn insert_db_info(data: Data) -> Result<(), String> {
    let response = MyRequest::post("/PIM/wp-admin/admin-ajax.php?action=problemas_edit")
        .json(&data)
        .send_no_parse()
        .await;
    match response {
...
pub async fn delete(id: usize) -> Result<(), String> {
    ...
    let response = MyRequest::post("/PIM/wp-admin/admin-ajax.php?action=problemas_delete")
        .body(&form)
        .send_no_parse()
        .await;
    match response {
